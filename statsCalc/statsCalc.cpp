...
// funcs
    void getData() {
        ...
        // match vars
        std::regex numMatchPattern(R"~(\d+)(,)~");
        std::smatch numMatch;
        do {
            std::regex_search(dataSetStr, numMatch, numMatchPattern);
            dataVector.push_back(std::stoi(numMatch[0]));
            dataSetStr.erase(numMatch.position(), numMatch.length() + 1);
        } while (std::regex_search(dataSetStr, numMatch, numMatchPattern) != false);
